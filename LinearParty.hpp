...
private:
    ...
    void randomSecretePoly(FieldType& s, int d, vector<FieldType>& polynomial);

    void expandNShareForPartition(int partitionIdx, vector<FieldType>& rShareNp,
    ...
    void spreadElms(vector<FieldType>& buffer, int nElements, int rootId);

    void scatterForDealers(vector<FieldType>& sendElms,
                           vector<FieldType>& recvElms, vector<int>& dealerIds);
    ...
    void gatherForDealers(vector<FieldType>& sendElms,
                          vector<FieldType>& recvElms, vector<int>& dealerIds);
    void scatterForAll(vector<FieldType>& sendElms, vector<FieldType>& recvElms);
    ...
    void robustBatchBroadcast(vector<FieldType>& sendElms,
                              vector<vector<FieldType> >& recvElms, int nElements,
                              vector<int>& dealerIds);
    ...
    bool singleShareRandom(int d, vector<FieldType>& shares, FieldType& checkVal);
    ...
    bool reconstructPublic(vector<FieldType>& shares, vector<FieldType>& results,
                           int degree);
    ...
    bool generateTuples(vector<vector<FieldType> >& a,
                        vector<vector<FieldType> >& b, vector<FieldType>& c);
    ...
    void linearInput(vector<vector<FieldType> >& recvInputs, int nInputs,
                     vector<int>& dealerIds);
...
static inline void fillInputSizes(int myId, int nParties,
                                  ArithmeticCircuit& circuit,  // input
                                  vector<int>& sizes)
{
    // output
...
static inline void lastNActiveParties(int lastN, const vector<bool>& mask,
...
static inline void firstNActiveParties(int firstN, const vector<bool>& mask,
                                       vector<int>& activeIds)
{
    int firstId = 0;
    int idx = 0;
...
template <class FieldType>
static inline void firstNActiveValues(int firstN, const vector<bool>& mask,
...
template <class FieldType>
static inline void lastNActiveValues(int lastN, const vector<bool>& mask,
                                     vector<FieldType>& totalValues,
                                     vector<FieldType>& activeValues)
{
    vector<int> activeIds;
...
template <class FieldType>
void LinearParty<FieldType>::makeField()
{
...
template <class FieldType>
void LinearParty<FieldType>::makeParties()
{
    ...
    _nPartiesInc = _nActiveParties;
    _nParties = _nPartiesInc - 1;
    _partyIds.resize(_nParties);
    for (int i = 0; i < _nParties; i++) {
        _partyIds[i] = _parties[i]->getID();
    }
...
template <class FieldType>
void LinearParty<FieldType>::makeAlpha()
{
...
template <class FieldType>
void LinearParty<FieldType>::makeBeta()
{
...
template <class FieldType>
void LinearParty<FieldType>::makePartition()
{
...
template <class FieldType>
LinearParty<FieldType>::LinearParty(int argc, char* argv[])
...
template <class FieldType>
void LinearParty<FieldType>::encodeFieldElts(vector<FieldType>& input,
        vector<byte>& output)
{
...
template <class FieldType>
void LinearParty<FieldType>::encodeFieldElt(FieldType& input,
        vector<byte>& output)
{
    int fieldByteSize = _field->getElementSizeInBytes();
...
template <class FieldType>
void LinearParty<FieldType>::decodeFieldElts(vector<byte>& input,
...
template <class FieldType>
void LinearParty<FieldType>::decodeFieldElt(vector<byte>& input,
        FieldType& output)
{
    int fieldByteSize = _field->getElementSizeInBytes();
...
template <class FieldType>
void LinearParty<FieldType>::readInputFromFile(string fileName)
{
...
template <class FieldType>
void LinearParty<FieldType>::spreadElms(vector<FieldType>& buffer,
...
template <class FieldType>
void LinearParty<FieldType>::prepareExpandedElms(
    vector<FieldType>& polynomial, vector<FieldType>& expandedElms)
{
    expandedElms.resize(_nPartiesInc);
...
template <class FieldType>
void LinearParty<FieldType>::robustBatchBroadcast(
    vector<FieldType>& sendElms,           // input
    ...
    int nElements, vector<int>& dealerIds)
{
    ...
    allExpandedElms(nDealers, vector<FieldType>(_nPartiesInc));
...
template <class FieldType>
void LinearParty<FieldType>::randomSecretePoly(FieldType& s, int degree,
...
template <class FieldType>
void LinearParty<FieldType>::scatterForDealersMulti(
    vector<vector<FieldType> >& sendElms, vector<vector<FieldType> >& recvElms,
    vector<int>& dealerIds)
{
    int nDealers = dealerIds.size();
    ...
    if (isDealer) {
        ...
        _baParty.sendAndRecvMsgs(sendMsgs, recvMsgs, msgSize, _partyIds, dealerIds);
        recvMsgs[_myId] = sendMsgs[_myId];
    } else {
        vector<int> writeList;
        _baParty.sendAndRecvMsgs(sendMsgs, recvMsgs, msgSize, writeList, dealerIds);
    }
...
template <class FieldType>
void LinearParty<FieldType>::scatterForDealers(vector<FieldType>& sendElms,
        ...
        vector<int>& dealerIds)
{
    ...
    if (isDealer) {
        for (int i = 0; i < _nPartiesInc; i++) {
            encodeFieldElt(sendElms[i], sendMsgs[i]);
        }
        _baParty.sendAndRecvMsgs(sendMsgs, recvMsgs, msgSize, _partyIds, dealerIds);
        recvMsgs[_myId] = sendMsgs[_myId];
    } else {
    ...
    for (int i = 0; i < nDealers; i++) {
        int dealerId = dealerIds[i];
        decodeFieldElt(recvMsgs[dealerId], recvElms[i]);
    }
    return;
}

template <class FieldType>
void LinearParty<FieldType>::gatherForDealers(vector<FieldType>& sendElms,
        vector<FieldType>& recvElms,
        vector<int>& dealerIds)
{
    int nDealers = dealerIds.size();
    ...
    if (isDealer) {
        _baParty.sendAndRecvMsgs(sendMsgs, recvMsgs, msgSize, dealerIds, _partyIds);
        recvMsgs[_myId] = sendMsgs[_myId];
        ...
        vector<int> readList;
        _baParty.sendAndRecvMsgs(sendMsgs, recvMsgs, msgSize, dealerIds, readList);
    }
...
template <class FieldType>
void LinearParty<FieldType>::scatterForAll(vector<FieldType>& sendElms,
        vector<FieldType>& recvElms)
{
    vector<int> dealers;
    for (int i = 0; i < _nPartiesInc; i++) {
...
template <class FieldType>
void LinearParty<FieldType>::scatterForAllMulti(
    vector<vector<FieldType> >& sendElms,
    vector<vector<FieldType> >& recvElms)
{
    vector<int> dealers;
    for (int i = 0; i < _nPartiesInc; i++) {
...
template <class FieldType>
bool LinearParty<FieldType>::checkPolynomial(vector<FieldType>& y, int degree,
        vector<FieldType>& g)
{
    bool happiness = true;
    int nPoints = y.size();
    ...
    for (int i = 0; i < nPoints; i++) {
        if (_activeMask[i]) {
            happiness &= (_eccAlpha.evalPolynomial(_alpha[i], g) == y[i]);
        }
    }
...
template <class FieldType>
bool LinearParty<FieldType>::singleShareSecrete(FieldType& s, int d,
        ...
        FieldType& checkVal)
{
    ...
    // -- every party applies HIM on received shares
    vector<FieldType> transformedShares(_nPartiesInc);
...
template <class FieldType>
bool LinearParty<FieldType>::singleShareRandom(int d, vector<FieldType>& shares,
        FieldType& checkVal)
{
    bool happiness = true;
    FieldType r = _field->Random();
...
template <class FieldType>
bool LinearParty<FieldType>::multipleShareRandom(
    const vector<int>& degrees, vector<vector<FieldType> >& shares)
{
    bool happiness = true;
...
// similarly create T random shares of 0 among parties
template <class FieldType>
bool LinearParty<FieldType>::singleShareZero(int d, vector<FieldType>& shares)
{
    bool happiness = true;
...
template <class FieldType>
FieldType LinearParty<FieldType>::reconstructPrivate(FieldType& share,
        int degree, int root)
{
    ...
    if (_myId == root) {
        ...
        vector<FieldType> shares(_nPartiesInc, _zero);
        shares[_myId] = share;

        recvMsgs.resize(_nParties);
...
template <class FieldType>
bool LinearParty<FieldType>::reconstructPublic(vector<FieldType>& shares,
        vector<FieldType>& results,
        int degree)
{
    int msgSize = _field->getElementSizeInBytes();
    ...
    // -- treat shares as a polynomial
    // -- expand to n points by evaluating at beta
    vector<FieldType> uSend(_nPartiesInc);
    for (int i = 0; i < _nPartiesInc; i++) {
        uSend[i] = _eccAlpha.evalPolynomial(_beta[i], shares);
    }

    // -- send to corresponding parties (all-to-all)
    vector<FieldType> uRecv(_nPartiesInc);
    ...
    vector<vector<byte> > recvMsgs(_nParties, vector<byte>(msgSize));
    ...
    // -- reconstruct g according to received
    // -- and coefficients are reconstruced values.
    vector<FieldType> recvF0(_nPartiesInc);
    recvF0[_myId] = f0;
    for (int i = 0; i < _nParties; i++) {
        if (!_activeMask[_parties[i]->getID()]) {
            continue;
...
template <class FieldType>
bool LinearParty<FieldType>::generateTuples(vector<vector<FieldType> >& a,
...
// TODO implement
template <class FieldType>
bool LinearParty<FieldType>::generateTuplesPE(vector<vector<FieldType> >& a,
        vector<vector<FieldType> >& b,
        vector<FieldType> c,
        vector<int>& elimIds)
{
...
template <class FieldType>
FieldType LinearParty<FieldType>::evalMultGate(
    int kingId, FieldType& xShare, FieldType& yShare, FieldType& aShareT,
    FieldType& aShareNp, FieldType& bShareT, FieldType& bShareNp,
    FieldType& cShareT, FieldType& d, FieldType& e)
{
    // -- all party computes (n-1)-share d_np := x_t + a_np
    // -- all party computes (n-1)-share e_np := y_t + b_np
    ...
    // -- all parties compute z_t := d*e - d*b_t - e*a_t + c_t
    FieldType zShare = reconsDE[0] * reconsDE[1] - reconsDE[0] * bShareT -
                       reconsDE[1] * aShareT + cShareT;
    return zShare;
}
...
template <class FieldType>
bool LinearParty<FieldType>::faultDetection(bool happiness)
{
    // in-active parties's bit is set to true
    vector<bool> recvBits(_nParties, true);
...
// TODO: implement
template <class FieldType>
void LinearParty<FieldType>::faultLocalizeConsistency(vector<int>& elimIds)
{
    cout << "fault localization for checking consistency not implemented yet"
         << endl;
    assert(false);
    return;
}
...
// TODO implement
template <class FieldType>
void LinearParty<FieldType>::faultLocalizeEvalSeg(vector<int>& elimIds)
{
...
template <class FieldType>
bool LinearParty<FieldType>::checkConsistency(int kingId,
        vector<FieldType>& vals,
        vector<int>& elimIds)
{
    bool happiness = true;
    int smallTp = (_nActiveParties - _bigT) / 2;
    ...
    paddedVals.resize(_nPartiesInc, *(_field->GetZero()));
    vector<FieldType> expandedShares(_nPartiesInc);
    ...
    if (recvElms.size() > 0) {
        // I'm a verifier
        for (int i = 0; i < _nPartiesInc - 1; i++) {
...
template <class FieldType>
void LinearParty<FieldType>::expandNShareForPartition(
    int partitionIdx, vector<FieldType>& rShareNp, vector<FieldType>& rShareT)
{
...
// expand a whole (n'-1)-share into 3 r-shares
template <class FieldType>
void LinearParty<FieldType>::expandNShare(vector<FieldType>& rShareNp,
...
template <class FieldType>
void LinearParty<FieldType>::extractNShare(vector<FieldType>& rShareT1,
        vector<FieldType>& rShareT2,
        vector<FieldType>& rShareT3,
...
template <class FieldType>
void LinearParty<FieldType>::eliminateParties(const vector<int>& elimIds)
{
    for (auto id : elimIds) {
...
template <class FieldType>
static inline vector<vector<FieldType> > transposeVectors(
    vector<vector<FieldType> >& m)
{
...
// Called QuadrupleShareRandom in the new paper
template <class FieldType>
bool LinearParty<FieldType>::generate4Tuples(
    vector<vector<FieldType> >& rTuples)
{
    bool happiness = true;
    FieldType r = _field->Random();
    rTuples.resize(4);
    ...
    vector<FieldType> f;
    vector<FieldType> fVals(_nPartiesInc);
    ...
    vector<int> dealers(nVerifiers);
    lastNActiveParties(nVerifiers, _activeMask, dealers);
    vector<FieldType> verifySharesT(_nPartiesInc);
    ...
    if (toVerifySharesT[0].size() > 0) {  // I'm a verifyer
        ...
        vector<FieldType> rShareNp;
        extractNShare(toVerifySharesT[0], toVerifySharesT[1], toVerifySharesT[2],
                      rShareNp);
...
// TODO: implement
template <class FieldType>
bool LinearParty<FieldType>::check4Consistency()
{
...
template <class FieldType>
bool LinearParty<FieldType>::evalSeg(vector<TGate>& circuitSeg,
                                     vector<int>& elimIds)
{
    bool happiness = true;
    ...
    for (auto gate : circuitSeg) {
        ...
        case MULT:
            ...
            _wireShares[gate.output] =
                evalMultGate(kingId, xShare, yShare, a[0][multOffset],
                             a[1][multOffset], b[0][multOffset], b[1][multOffset],
                             c[multOffset], kingDs[multOffset], kingEs[multOffset]);
            multOffset++;
            break;
        default:
...
static inline int collectInputIds(vector<int>& inputSizes,
                                  vector<int>& inputParties, int maxBatch)
{
    ...
    while (firstInputParty < nPartiesInc && inputSizes[firstInputParty] == 0) {
        firstInputParty++;
    }
...
template <class FieldType>
static inline void storeRecvDealerInputs(vector<FieldType>& totalInputs,
...
template <class FieldType>
void LinearParty<FieldType>::linearInput(vector<vector<FieldType> >& recvInputs,
        int nInputs, vector<int>& dealerIds)
{
    ...
    // -- All dealers batch broadcast its T differences
    vector<vector<FieldType> > recvOffsets(_nPartiesInc);
...
void LinearParty<FieldType>::InputPhase()
{
    ...
    vector<vector<FieldType> > receivedInputShares(_nPartiesInc);
    ...
    // vector< vector<FieldType> > receivedInputsClear(_nPartiesInc);
    ...
    vector<int> offsets(_nPartiesInc);
...
void LinearParty<FieldType>::EvalPhase()
{
    ...
    int nGatesLeft = nTotalGates - _circuit.getNrOfInputGates() -
                     _circuit.getNrOfOutputGates();
    int gateIdx = 0;
    int nMults = 0;
...
void LinearParty<FieldType>::OutputPhase()
{
    ...
    cout << "---- there are " << nOutputGates << " output gates" << endl;
    ...
    for (auto g : gates) {
        if (g.gateType == OUTPUT) {
            ...
            if (rootId == _myId) {
                cout << _myId << " : wire " << g.input1 << " is " << output << endl;
            }
        }
    }
...
void LinearParty<FieldType>::runOffline()
{
    ...
    cout << "running singleShareRandom in " << nBatches << " batches" << endl;
    ...
    cout << "running singleShareZero in " << nBatches << " batches" << endl;
