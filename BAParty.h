...
exchangeBit(bool sendBit, vector<bool>& recvBits){
  ...
  for(int i=1; i<_nThread; i++){
    threads[i-1] = thread(&BAParty::exchangeBitWorker, this,
...
spreadBit(bool sendBit, int kingId){
  ...
  if(_myId == kingId){
    ...
    vector<thread> threads(_nThread-1);
    for(int i=1; i<_nThread; i++){
      threads[i-1] = thread(&BAParty::spreadBitWorker, this,
                          sendBit, i, _nThread);
    }
...
commiteeRecvBits(const vector<char>& commitee_mask, const int* QCount,
  ...
  vector<thread> threads(_nThread-1);
  for(int i=1; i<_nThread; i++){
    threads[i-1] = thread(&BAParty::gatherBitWorker, this,
...
broadcastMsgForAll(vector<byte>& sendMsg, // input
  ...
  for(int i=1; i<_nThread; i++){
    threads[i-1] = thread(&BAParty::exchangeMsgWorker, this,
...
spreadMsg(vector<byte>& msg, int msgSize, int rootId){
  ...
  if (rootId == _myId) {
    ...
    vector<thread> threads(_nThread-1);
    for (int i = 1; i < _nThread; i++) {
      threads[i-1] = thread(&BAParty::spreadMsgWorker, this,
                          ref(msg), i, _nThread);
    }
...
scatterMsg(vector< vector<byte> >& sendMsgs,
  ...
  if (rootId == _myId) {
    ...
    vector<thread> threads(_nThread-1);
    for (int i = 1; i < _nThread; i++) {
      threads[i-1] = thread(&BAParty::scatterMsgWorker, this,
...
gatherMsg(vector<byte>& sendMsg, vector< vector<byte> >& recvMsgs,
  ...
  if(rootId == _myId){
    ...
    vector<thread> threads(_nThread-1);
    for(int i=1; i<_nThread; i++){
      threads[i-1] = thread(&BAParty::gatherMsgWorker, this,
...
sendAndRecvMsgs(vector< vector<byte> >& sendMsgs,
  ...
  vector<thread> threads(_nThread-1);
  for(int i=1; i<_nThread; i++){
    threads[i-1] = thread(&BAParty::sendAndRecvMsgsWorker, this,
