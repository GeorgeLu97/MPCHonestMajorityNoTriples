...
private:
    ...
    void exchangeBitWorker(bool sendBit, vector<bool>& recvBits, int threadId,
                           int nThread);
    void exchangeMsgWorker(vector<byte>& sendMsg, vector<vector<byte> >& recvMsgs,
    ...
    void gatherMsgWorker(vector<vector<byte> >& recvMsgs, int msgSize,
                         int threadId, int nThread);
    void scatterMsgWorker(vector<vector<byte> >& sendMsgs, int msgSize,
                          int threadId, int nThread);
    ...
    bool universal_rounds(bool b, int* D, vector<bool>& buffer,
                          vector<bool>& buffer2);
...
// set protocol properties: (should be known before running!)
template <class FieldType>
void BAParty<FieldType>::setParties(
    vector<shared_ptr<ProtocolPartyData> >& parties, int myId)
{
...
template <class FieldType>
void BAParty<FieldType>::setHIM(HIM<FieldType>& common_HIM)
{
...
template <class FieldType>
void BAParty<FieldType>::setAlphaBeta(vector<FieldType>& alpha,
...
template <class FieldType>
void BAParty<FieldType>::setNumThreads(int numThreads)
{
    _nThread = numThreads;
    return;
}
...
template <class FieldType>
void BAParty<FieldType>::getRemainingParties(
...
template <class FieldType>
void BAParty<FieldType>::setPartyInactive(int id)
{
    assert(id != _myId);
    int nParties = _partySocket.size();
    for (int i = 0; i < nParties; i++) {
...
template <class FieldType>
void BAParty<FieldType>::exchangeBitWorker(bool sendBit, vector<bool>& recvBits,
        int threadId, int nThread)
{
    int nParties = _partySocket.size();
...
template <class FieldType>
void BAParty<FieldType>::exchangeBit(bool sendBit, vector<bool>& recvBits)
{
    recvBits.resize(_partySocket.size());

    vector<thread> threads(_nThread - 1);
    for (int i = 1; i < _nThread; i++) {
        threads[i - 1] = thread(&BAParty::exchangeBitWorker, this, sendBit,
                                ref(recvBits), i, _nThread);
    }
...
template <class FieldType>
void BAParty<FieldType>::spreadBitWorker(bool sendBit, int threadId,
        int nThread)
{
    int nParties = _partySocket.size();
    char sendByte = sendBit ? 1 : 0;
...
template <class FieldType>
void BAParty<FieldType>::gatherBitWorker(vector<bool>& recvBits, int threadId,
        int nThread)
{
    int nParties = _partySocket.size();
    vector<char> recvBytes(nParties);
...
template <class FieldType>
void BAParty<FieldType>::scatterMsgWorker(vector<vector<byte> >& sendMsgs,
        int msgSize, int threadId,
        int nThread)
{
    int nParties = _partySocket.size();
    // communicate with other parties
...
template <class FieldType>
bool BAParty<FieldType>::spreadBit(bool sendBit, int kingId)
{
    int nParties = _partySocket.size();
    if (_myId == kingId) {
        ...
        for (int i = 1; i < _nThread; i++) {
            threads[i - 1] =
                thread(&BAParty::spreadBitWorker, this, sendBit, i, _nThread);
        }
...
template <class FieldType>
void BAParty<FieldType>::printActiveParties()
{
    int nParties = _partySocket.size();
...
template <class FieldType>
void BAParty<FieldType>::commiteeSendBit(const vector<char>& commitee_mask,
...
template <class FieldType>
void BAParty<FieldType>::commiteeRecvBits(const vector<char>& commitee_mask,
        const int* QCount, int myCommitee,
        vector<bool>& buffer)
{
    ...
    for (int i = 1; i < _nThread; i++) {
        threads[i - 1] =
            thread(&BAParty::gatherBitWorker, this, ref(buffer), i, _nThread);
    }
...
template <class FieldType>
bool BAParty<FieldType>::universal_rounds(bool b, int* D, vector<bool>& buffer,
        vector<bool>& buffer2)
{
...
template <class FieldType>
bool BAParty<FieldType>::consensus_base(bool b)
{
    int smallT = (_nActiveParties - 1) / 3;
...
template <class FieldType>
int BAParty<FieldType>::splitCommitee(vector<char>& commitee_mask,
                                      int* QCount)
{
    int curCommitee = 0;
...
template <class FieldType>
void BAParty<FieldType>::mapCommitee(const vector<char>& commitee_mask,
                                     const int* QCount, int myCommitee)
{
    int nParties = _partySocket.size();
    for (int i = 0; i < nParties; i++) {
...
template <class FieldType>
void BAParty<FieldType>::unmapCommitee(const vector<char>& commitee_mask,
                                       const int* QCount, int myCommitee)
{
    int nParties = _partySocket.size();
    for (int i = 0; i < nParties; i++) {
...
template <class FieldType>
void BAParty<FieldType>::printCommitee(vector<char>& commitee_mask,
                                       int myCommitee)
{
    int nParties = _partySocket.size();
...
template <class FieldType>
bool BAParty<FieldType>::consensus(bool b)
{
...
template <class FieldType>
void BAParty<FieldType>::broadcastBit(bool& b, int rootId)
{
...
template <class FieldType>
void BAParty<FieldType>::spreadMsgWorker(vector<byte>& msg, int threadId,
        int nThread)
{
    int nParties = _partySocket.size();
    for (int i = threadId; i < nParties; i += nThread) {
...
template <class FieldType>
void BAParty<FieldType>::exchangeMsgWorker(vector<byte>& sendMsg,
...
template <class FieldType>
void BAParty<FieldType>::broadcastMsgForAll(vector<byte>& sendMsg,  // input
        vector<vector<byte> >& recvMsgs,
        int msgSize)
{
    vector<thread> threads(_nThread - 1);
    ...
    for (int i = 1; i < _nThread; i++) {
        threads[i - 1] = thread(&BAParty::exchangeMsgWorker, this, ref(sendMsg),
...
template <class FieldType>
void BAParty<FieldType>::broadcastMsg(vector<byte>& msg, int msgSize,
                                      int rootId)
{
...
template <class FieldType>
void BAParty<FieldType>::spreadMsg(vector<byte>& msg, int msgSize, int rootId)
{
    int nParties = _partySocket.size();
    ...
    if (rootId == _myId) {
        ...
        for (int i = 1; i < _nThread; i++) {
            threads[i - 1] =
                thread(&BAParty::spreadMsgWorker, this, ref(msg), i, _nThread);
        }
        ...
        msg.resize(msgSize);
        for (int i = 0; i < nParties; i++) {
            if (!_activeMask[i] || _partySocket[i]->getID() != rootId) {
                continue;
            }
...
template <class FieldType>
void BAParty<FieldType>::scatterMsg(vector<vector<byte> >& sendMsgs,
                                    vector<byte>& recvMsg, int msgSize,
                                    int rootId)
{
    int nParties = _partySocket.size();
    ...
    if (rootId == _myId) {
        ...
        vector<thread> threads(_nThread - 1);
        for (int i = 1; i < _nThread; i++) {
            threads[i - 1] = thread(&BAParty::scatterMsgWorker, this, ref(sendMsgs),
                                    msgSize, i, _nThread);
        }
...
template <class FieldType>
void BAParty<FieldType>::gatherMsgWorker(vector<vector<byte> >& recvMsgs,
        int msgSize, int threadId,
        int nThread)
{
    int nParties = _partySocket.size();
...
template <class FieldType>
void BAParty<FieldType>::gatherMsg(vector<byte>& sendMsg,
                                   vector<vector<byte> >& recvMsgs, int msgSize,
                                   int rootId)
{
    int nParties = _partySocket.size();
    ...
    if (rootId == _myId) {
        ...
        vector<thread> threads(_nThread - 1);
        for (int i = 1; i < _nThread; i++) {
            threads[i - 1] = thread(&BAParty::gatherMsgWorker, this, ref(recvMsgs),
                                    msgSize, i, _nThread);
        }
...
template <class FieldType>
void BAParty<FieldType>::sendAndRecvMsgsWorker(vector<vector<byte> >& sendMsgs,
        vector<vector<byte> >& recvMsgs,
        int msgSize, int threadId,
        int nThread,
        const vector<char> opMask)
    ...
    for (int i = threadId; i < nParties; i += nThread) {
        ...
        case 3:  // both
            ...
            if (_myId < partyId) {  // write before read
                _partySocket[i]->getChannel()->write(sendMsgs[partyId].data(),
                                                     msgSize);
                _partySocket[i]->getChannel()->read(recvMsgs[partyId].data(),
                                                    msgSize);
...
template <class FieldType>
void BAParty<FieldType>::sendAndRecvMsgs(vector<vector<byte> >& sendMsgs,
        vector<vector<byte> >& recvMsgs,
        int msgSize,
        const vector<int>& writeList,
        const vector<int>& readList)
{
    ...
    for (int i = 1; i < _nThread; i++) {
        threads[i - 1] =
            thread(&BAParty::sendAndRecvMsgsWorker, this, ref(sendMsgs),
                   ref(recvMsgs), msgSize, i, _nThread, ref(opMask));
    }
    sendAndRecvMsgsWorker(sendMsgs, recvMsgs, msgSize, 0, _nThread, opMask);
    for (int i = 1; i < _nThread; i++) {
        threads[i - 1].join();
    }
    return;
